...
async def trace(trace_id: str, actor=Depends(require_actor), db: AsyncSession = Depends(get_db)):
    rows = (await db.execute(select(AuditEvent).where(AuditEvent.trace_id == trace_id).order_by(AuditEvent.id.asc()))).scalars().all()
    return {'items': [
        {
            'id': r.id,
            'created_at': r.created_at.isoformat(),
