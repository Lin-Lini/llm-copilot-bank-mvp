...
def _partial_score(item: dict[str, Any], pq: PlannedQuery) -> float:
    rel = 0.56 * float(item.get('lex_sim', 0.0)) + 0.44 * float(item.get('sem_sim', 0.0))
...
async def hybrid_search(db: AsyncSession, query: str, top_k: int = 5) -> list[dict[str, Any]]:
    ...
    for pq in plans:
        ...
        for item in candidates.values():
            if pq.label not in item['matched_queries']:
                continue
            item['score'] = max(item['score'], _partial_score(item, pq)) + 0.08 * _partial_score(item, pq)
    ...
    ranked = sorted(all_items, key=lambda x: (float(x.get('rerank') or 0.0), x['doc_id'], x['section']), reverse=True)
