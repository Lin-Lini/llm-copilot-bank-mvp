...
- код можно изучать, запускать, изменять и использовать в некоммерческих целях;
- проект подходит для личного использования, обучения, исследований, тестирования и портфолио;
- коммерческое использование по условиям этой лицензии не разрешено;
- для коммерческого использования требуется отдельное разрешение правообладателя.

Полный текст лицензии приведён в файле `LICENSE`.

Важно: проект является **source-available**, но не **open source** в строгом смысле, поскольку лицензия ограничивает коммерческое использование.
